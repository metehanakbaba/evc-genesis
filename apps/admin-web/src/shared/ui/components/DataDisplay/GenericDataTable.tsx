...
import type { ActionButton, GridItem } from './GenericDataGrid';
...
const TableHeader = <T extends GridItem>({
  ...
  const isAllSelected =
    selectedItems && selectedItems.size === totalItems && totalItems > 0;
  const isIndeterminate =
    selectedItems && selectedItems.size > 0 && selectedItems.size < totalItems;
            ...
            {actions
              ...
              .map((action, actionIndex) => {
                ...
                return (
                  ...
                  >
                    <ActionIcon
                      className={`w-4 h-4 ${getTableActionIconAnimation(action.label, isMainAction)}`}
                    />
                    {isMainAction && (
...
export const GenericDataTable = <T extends GridItem>({
  ...
  if (!isLoading && items.length === 0) {
    return (
      <div
        className={`bg-gray-800/40 border border-gray-700/50 rounded-2xl overflow-hidden backdrop-blur-xl ${className}`}
      >
        {emptyState || (
...
const getTableActionIconAnimation = (
  actionLabel: string,
